...
async function printETHBalance({ address, name }) {
  console.log(`${name} MOVR balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function deposit({ currency, amount }) {
  ...
  if (currency === 'movr') {
    ...
    insertDepositDB(deposited, currency, amount)
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 1285) {
        console.log(`Transaction submitted through the relay. View transaction on Moonriver scan https://blockscout.moonriver.moonbeam.network/`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 1285) {
          console.log(`View transaction on moonriver https://blockscout.moonriver.moonbeam.network/`)
        } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'movr', amount = '0.1' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/MOVRSmashnado.json')).json()
  ...
  if (!(netId === 1285)) {
    throw new Error('You did not connect to MOONRIVER network. Please make sure you connected to MOONRIVER Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to MOONRIVER Mainnet.')
  erc20 = currency !== 'movr' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'movr', amount: '0.1' }
  //await init(instance)
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient MOVR Balance in your wallet to deposit.', 'error')
      return false;
    }
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account MOVR balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
